...
describe("task tool: independent lifecycle verbs", () => {
  ...
  it.live("block operation transitions task to blocked", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const session = yield* Session.Service
        const reg = yield* TaskRegistry.Service
        const sess = yield* session.create({ title: "Test" })
        const t = yield* reg.create({ session_id: sess.id, summary: "blocked task" })
        const info = yield* TaskTool
        const tool = yield* info.init()
        yield* tool.execute({ operation: { action: "block", id: t.id, event_summary: "waiting on review" } }, ctx(sess.id))
        const after = yield* reg.get({ session_id: sess.id, id: t.id })
  ...
  it.live("unblock operation transitions task back to open", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const session = yield* Session.Service
        const reg = yield* TaskRegistry.Service
        const sess = yield* session.create({ title: "Test" })
        const t = yield* reg.create({ session_id: sess.id, summary: "unblock task" })
        yield* reg.block({ session_id: sess.id, id: t.id })
        const info = yield* TaskTool
        const tool = yield* info.init()
        yield* tool.execute({ operation: { action: "unblock", id: t.id, event_summary: "ready again" } }, ctx(sess.id))
        const after = yield* reg.get({ session_id: sess.id, id: t.id })
  ...
  it.live("done operation transitions task to done", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const session = yield* Session.Service
        const reg = yield* TaskRegistry.Service
        const sess = yield* session.create({ title: "Test" })
        const t = yield* reg.create({ session_id: sess.id, summary: "done task" })
        const info = yield* TaskTool
        const tool = yield* info.init()
        yield* tool.execute({ operation: { action: "done", id: t.id, event_summary: "all complete" } }, ctx(sess.id))
        const after = yield* reg.get({ session_id: sess.id, id: t.id })
        expect(after?.status).toBe("done")
        expect(after?.ended_at).toBeDefined()
      }),
    ),
  )

  it.live("abandon operation transitions task to abandoned", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const session = yield* Session.Service
        const reg = yield* TaskRegistry.Service
        const sess = yield* session.create({ title: "Test" })
        const t = yield* reg.create({ session_id: sess.id, summary: "abandoned task" })
        const info = yield* TaskTool
        const tool = yield* info.init()
        yield* tool.execute({ operation: { action: "abandon", id: t.id, event_summary: "dropped" } }, ctx(sess.id))
        const after = yield* reg.get({ session_id: sess.id, id: t.id })
  ...
  it.live("get operation returns single task", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const session = yield* Session.Service
        const reg = yield* TaskRegistry.Service
        const sess = yield* session.create({ title: "Test" })
        const t = yield* reg.create({ session_id: sess.id, summary: "get me" })
        const info = yield* TaskTool
        const tool = yield* info.init()
        const result = yield* tool.execute({ operation: { action: "get", id: t.id } }, ctx(sess.id))
        expect(result.output).toContain(t.id)
        expect(result.output).toContain("get me")
      }),
    ),
  )

  it.live("start operation transitions task to in_progress", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const session = yield* Session.Service
        const reg = yield* TaskRegistry.Service
        const sess = yield* session.create({ title: "Test" })
        const t = yield* reg.create({ session_id: sess.id, summary: "start me" })
        const info = yield* TaskTool
        const tool = yield* info.init()
        yield* tool.execute({ operation: { action: "start", id: t.id } }, ctx(sess.id))
        const after = yield* reg.get({ session_id: sess.id, id: t.id })
        expect(after?.status).toBe("in_progress")
      }),
    ),
  ...
  it.live("rename operation updates summary", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const session = yield* Session.Service
        const reg = yield* TaskRegistry.Service
        ...
        const info = yield* TaskTool
        const tool = yield* info.init()
        yield* tool.execute({ operation: { action: "rename", id: t.id, summary: "new" } }, ctx(sess.id))
        const after = yield* reg.get({ session_id: sess.id, id: t.id })
